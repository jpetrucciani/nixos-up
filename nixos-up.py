...
def ask_github() -> list[str]:
    github = (
        input(
            "Would you like to load the pubkeys for a github user? (hit enter to skip) "
        )
        .strip()
        .lower()
    )
    if github:
        try:
            return [
                x
                for x in requests.get(f"https://github.com/{github}.keys").text.split(
                    "\n"
                )
                if x
            ]
        except Exception:
            print(f"failed to load keys for user '{github}'")
    return []
...
def partition_name(disk: str, partition: int) -> str:
    if disk.startswith("sd"):
        ...
        print(
            "Warning: this type of device driver has not been thoroughly tested with nixos-up, and its partition naming scheme may differ from what we expect. Please open an issue at https://github.com/jpetrucciani/nixos-up/issues."
        )
        return f"{disk}{partition}"
...
def uncomment(_config: str, option: str) -> str:
    return _config.replace(f"# {option}", option)


# disable firewall by default
config = uncomment(config, "networking.firewall.enable = false;")

# enable ssh by default
config = uncomment(config, "services.openssh.enable = true;")

# enable copy system config
...
_keys = "\n".join([f'"{x}"' for x in ssh_keys])

config = re.sub(
    r" *# Define a user account\..*\n( *# .*\n)+",
    "\n".join(
        [
            ...
            f"    openssh.authorizedKeys.keys = [{_keys}];",
            "  };",
            "",
...
print("nixos-up complete! reboot now and unmount the installation media")
